...
exports.genre_list = function(req, res, next) {
    Genre.find()
        .exec(function(err, list_genre){
            ...
            res.render('genre_list', {title: 'Genere List', genre_list: list_genre})
        })
};
...
exports.genre_create_post =  [
    ...
    body('name', 'Genre must be minimum 3 chars long').isLength({ min: 3 }).trim().escape(),
...
exports.genre_update_post = function(req, res, next) {
    Genre.findByIdAndUpdate(req.params.id, new Genre({
      name: req.body.name,
      _id: req.params.id,
    }), function(err, resp){
      if(err) return next(err)
